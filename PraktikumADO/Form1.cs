    ...
    public partial class Form1 : Form
    {
        ...
        private void btnUpdateSKS_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string query = "UPDATE MataKuliah SET SKS WHERE KodeMK='IF210101'";
                cmd = new SqlCommand(query, conn);

                int hasil = cmd.ExecuteNonQuery();
                MessageBox.Show("Data SKS Berhasil di Update.baris terpengaruh" + hasil);
                conn.Close();
            }
            catch ( Exception ex)
            {
        ...
        private void btnInsertProdi_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int hasil = cmd.ExecuteNonQuery();
                MessageBox.Show("Data Prodi Berhasil di tambahkan. baris terpengaruh" + hasil);
                conn.Close();
            }
            catch(Exception ex)
            {
